...
Applies to Main (non-class) Overloaded procedures (which shows procedure overloading can be handled by the AppGen in general for templates generating code in other languages and could be handled by the Clarion and ABC templates with some modification), Class Overloaded Methods and Interface Overloaded Methods.
